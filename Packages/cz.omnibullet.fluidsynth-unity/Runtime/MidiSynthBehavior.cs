            ...
            while (dataPosition < dataLength)
            {
                ...
                for (var c = 0; c < channels; c++)
                {
                    ...
                    for (int i = start + c; i < end; i += channels)
                    {
                        ...
                        // Cela permet de mixer le son du synthé avec les autres sons (AudioSource) au lieu de les muter.
                    ...
                    if (preset_zone.Index >= 0)
                    {
                        ...
                        foreach (HiZone inst_zone in inst.Zone)
                        {
                                ...
                                inst_zone.VelHi >= vel)
                            {
                                ...
                                // Instrument modulators -supersede- existing (default) modulators.  SF2.01 page 69, 'bullet' 6
                                foreach (HiMod mod1 in mod_list)
